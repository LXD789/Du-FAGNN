...
# batch_size = 4096,1024,512(最佳——2021.4.23）,256

# flags.DEFINE_integer('input_dim', 300, 'Dimension of input.')
flags.DEFINE_integer('input_dim', 288, 'Dimension of input.')  # 256+32=288,512
...
for epoch in range(FLAGS.epochs):
    ...
    print("Epoch:", '%04d' % (epoch + 1), "train_loss=", "{:.5f}".format(train_loss),
          "train_acc=", "{:.5f}".format(train_acc), "test_acc=", "{:.5f}".format(test_acc),
          "time=", "{:.5f}".format(time.time() - t), "best_acc=", "{:.5f}".format(best_acc))
