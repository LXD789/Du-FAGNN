...
# 此部分：下载nltk库中的停用词、设置语言为英语并显示停用词(这部分代码用于twitter英文数据集)
nltk.download()
nltk.download('stopwords')   # 下载‘stopwords’
stop_words = set(stopwords.words('english'))
print("stop_words:", stop_words)

# 此部分：下载nltk库中的停用词、设置语言为汉语并显示停用词（这部分代码用于微博数据集）——2020.11.25
...
# -------------------------------------------------------------------------------------------------------
# 此部分供分类为0和1的twitter15数据集使用。后期的数据集能否使用另行研究。————2020.10.26

# with open('data/corpus/' + dataset + '-new.clean.txt', 'w') as f:
#     f.write(clean_corpus_str)


path3 = "data/corpus/twitter15-new.clean.txt"
path4 = "data/corpus/twitter15.clean.txt"
...
# -------------------------------------------------------------------------------------------------------
# 此部分供分为4类的twitter-v2数据集使用。————2020.10.26
...
# temp = []
# space_label_id = []  # 存放空行对应的id
# with open('data/corpus/twitter-v2-new.clean.txt', "r", encoding="utf-8") as c:
#     with open('data/corpus/twitter-v2.clean.txt', "w", encoding="utf-8") as c1:
#         lines = c.readlines()
#         b = 0
#         for line in lines:
#             if line != "\n":
...
#
# # 需要删除数据集文本文件中空行对应的label文件中的相应行
# with open('data/twitter-v2-old.txt', 'r', encoding='utf-8') as s:
#     with open('data/twitter-v2-temp.txt', 'w', encoding='utf-8') as n:
#         s_lines = s.readlines()
#         for s_line in s_lines:
#             label_id = s_line.split("\t")[0]
#             if len(space_label_id) != 0:
...
#
# # 在删除空行的label之后，将label文件中的id刷新
# with open('data/twitter-v2-temp.txt', 'r', encoding='utf-8') as t:
#     with open('data/twitter-v2.txt', 'w', encoding='utf-8') as su:
#         t_lines = t.readlines()
#         j = 0
#         for t_line in t_lines:
#             # t_line.split("\t")[0] = str(j)
