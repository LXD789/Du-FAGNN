...
# settings设置
datasets = ['mr', 'ohsumed', 'R8', 'R52', 'TREC', 'ag_news', 'WebKB', 'SST1', 'SST2', 'twitter15', 'twitter-v2']
...
# with open('glove.6B.' + str(word_embeddings_dim) + 'd.txt', 'r', errors='ignore') as f:
#     # 打开glove.6B.300d.txt文件
#     for line in f.readlines():
#         # 每次循环取一行文本，对每行文本进行分割，并向词嵌入字典中添加：
#         #    以当前行的第一个元素为键、第二个元素到最后一个元素为值的键值对。
#         data = line.split()
#         word_embeddings[str(data[0])] = list(map(float, data[1:]))
# 测试
# print("word_embeddings[str(data[0])]:", word_embeddings[str(data[0])])

with open('twitter15-newbert-all.txt', 'r', encoding='utf-8')as tnb:
